...
export function MolecularBackground({
  ...
  const initParticles = (width: number, height: number) => {
    ...
    const particleCount = Math.max(
      30,
      Math.min(Math.floor((width * height) / 12000), 80),
    );
    ...
    particles.forEach((particle, i) => {
      particle.connections.forEach((j) => {
        ...
        if (distance < maxDistance) {
          ...
          if (
            particle.type === "attractor" &&
            connectedParticle.type === "attractor"
          ) {
      ...
      particles.forEach((otherParticle, otherIndex) => {
        if (index !== otherIndex) {
          ...
          if (distance < 120) {
            ...
            if (
              particle.type === "attractor" &&
              otherParticle.type === "attractor"
            ) {
              // Attractors attract each other
              force = 0.025;
            } else if (
              particle.type === "repulsor" &&
              otherParticle.type === "repulsor"
            ) {
